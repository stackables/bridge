...
- **Abort discipline**: `resolveWiresAsync` and `createShadowArray` now check `signal.aborted` / `signal.throwIfAborted()` to halt execution immediately when a client disconnects.
