...
- Formalized engine-internal cache fields on `NodeRef` (`_key`) and the pull-wire variant of `Wire` (`_simplePullRef`) in `types.ts` — eliminates `(as any)` casts that previously looked like unsafe mutations of AST nodes
