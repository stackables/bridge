...
class CodegenContext {
  ...
  compile(): CompileResult {
    ...
    lines.push(`      let t; const timeout = new Promise((_, rej) => { t = setTimeout(() => rej(new Error("Tool timeout")), __timeoutMs); });`);
    lines.push(`      try { return await Promise.race([p, timeout]); } finally { clearTimeout(t); }`);
    lines.push(`    }`);
    lines.push(`    return p;`);
    lines.push(`  }`);
  ...
  private emitToolCall(
    ...
    if (!toolDef) {
      ...
      if (mode === "fire-and-forget") {
        ...
        lines.push(
          `  try { ${tool.varName} = await __call(tools[${JSON.stringify(tool.toolName)}], ${inputObj}); } catch (_e) { ${tool.varName}_err = true; }`,
        );
      } else {
        lines.push(
    ...
    if (onErrorWire) {
      ...
      lines.push(`  let ${tool.varName}, ${tool.varName}_err = false;`);
      lines.push(
        `  try { ${tool.varName} = await __call(tools[${JSON.stringify(fnName)}], ${inputObj}); } catch (_e) { ${tool.varName}_err = true; }`,
      );
    } else {
      lines.push(
