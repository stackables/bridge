import { materializeShadows as _materializeShadows } from "./materializeShadows.js";
import { resolveWires as _resolveWires } from "./resolveWires.js";
import { schedule as _schedule } from "./scheduleTools.js";
import { internal } from "./tools/index.js";
import { isOtelActive, otelTracer, SpanStatusCodeEnum, toolCallCounter, toolDurationHistogram, toolErrorCounter, TraceCollector, } from "./tracing.js";
import { BREAK_SYM, BridgeAbortError, BridgePanicError, CONTINUE_SYM, isPromise, MAX_EXECUTION_DEPTH, } from "./tree-types.js";
import { pathEquals, roundMs, sameTrunk, TRUNK_KEY_CACHE, trunkKey, UNSAFE_KEYS, } from "./tree-utils.js";
import { SELF_MODULE } from "./types.js";
import { raceTimeout } from "./utils.js";
export class ExecutionTree {
    ...
    elementTrunkKey;
    constructor(trunk, document, toolFns, 
    /**
    ...
    callTool(toolName, fnName, fnImpl, input) {
        ...
        return otelTracer.startActiveSpan(`bridge.tool.${toolName}.${fnName}`, { attributes: metricAttrs }, async (span) => {
            ...
            try {
                ...
                const result = timeoutMs > 0 && isPromise(toolPromise)
    ...
    pullSingle(ref, pullChain = new Set()) {
        ...
        const key = (ref[TRUNK_KEY_CACHE] ??= trunkKey(ref));
        // ── Cycle detection ─────────────────────────────────────────────
    ...
    async pullOutputField(path, array = false) {
        ...
        if (resolved === BREAK_SYM || resolved === CONTINUE_SYM)
            return [];
        return this.createShadowArray(resolved);
    }
    /**
     ...
     */
    async resolveNestedField(prefix) {
     ...
     */
    materializeShadows(items, pathPrefix) {
