import type { BridgeDocument, Instruction } from "./types.ts";
 ...
 * ```
 */
export function mergeBridgeDocuments(
  ...docs: BridgeDocument[]
): BridgeDocument {
  if (docs.length === 0) {
    return { instructions: [] };
  ...
  for (const doc of docs) {
    for (const inst of doc.instructions) {
      ...
      if (key) {
        if (seenDefs.has(key)) {
          throw new Error(
            `Merge conflict: duplicate ${key.replace(":", " '")}' across bridge documents.`,
          );
        }
        seenDefs.add(key);
      }
 ...
 */
function resolveVersion(docs: BridgeDocument[]): string | undefined {
  ...
  for (const doc of docs) {
    ...
    if (best !== undefined && major !== bestMajor) {
      throw new Error(
        `Cannot merge bridge documents with different major versions: ` +
          `${best} vs ${doc.version}. ` +
