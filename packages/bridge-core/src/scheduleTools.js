...
import { isPromise } from "./tree-types.js";
import { trunkKey, sameTrunk, setNested } from "./tree-utils.js";
import { lookupToolFn, resolveToolDefByName, resolveToolWires, resolveToolSource, } from "./toolLookup.js";
// ── Helpers ─────────────────────────────────────────────────────────────────
/** Derive tool name from a trunk. */
function getToolName(target) {
 ...
 */
export function schedule(ctx, target, pullChain) {
    ...
    if (ctx.parent) {
        const forkWires = ctx.bridge?.wires.filter((w) => sameTrunk(w.to, target)) ?? [];
        ...
        const hasTransitiveElementSource = target.module === "__local" &&
            forkWires.some((w) => {
                ...
                return (ctx.bridge?.wires.some((iw) => sameTrunk(iw.to, srcTrunk) && "from" in iw && !!iw.from.element) ?? false);
            });
    ...
    const baseWires = baseTrunk
        ? (ctx.bridge?.wires.filter((w) => !("pipe" in w) && sameTrunk(w.to, baseTrunk)) ?? [])
        : [];
    ...
    if (!hasAsync) {
        return scheduleFinish(ctx, target, toolName, groupEntries, values, baseTrunk);
    }
    return Promise.all(values).then((resolved) => scheduleFinish(ctx, target, toolName, groupEntries, resolved, baseTrunk));
}
// ── Schedule finish ─────────────────────────────────────────────────────────
/**
 ...
 * See docs/performance.md (#12).
 */
export function scheduleFinish(ctx, target, toolName, groupEntries, resolvedValues, baseTrunk) {
    const input = {};
    const resolved = [];
    ...
    const handleVersion = ctx.handleVersionMap.get(trunkKey(target)) ??
 ...
 */
export async function scheduleToolDef(ctx, toolName, toolDef, wireGroups, pullChain) {
