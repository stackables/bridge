...
export function coerceConstant(raw) {
...
export function setNested(obj, path, value) {
    for (let i = 0; i < path.length - 1; i++) {
        ...
        if (typeof obj !== "object" || obj === null) {
            throw new Error(`Cannot set nested property: value at "${key}" is not an object`);
        }
    }
    if (path.length > 0) {
 ...
 * See docs/performance.md (#11).
 */
export function getSimplePullRef(w) {
    if ("from" in w) {
        const cached = w[SIMPLE_PULL_CACHE];
        if (cached !== undefined)
            return cached;
        const ref = !w.safe &&
