/**
 * Unit tests for wire resolution.
 *
 * Tests expression evaluation, fallback gates, and catch handlers.
 */
...
import {
  ...
  applyFallbackGates,
  applyCatch,
} from "../src/resolveWiresSources.ts";
...
function makeWire(sources: Wire["sources"], opts: Partial<Wire> = {}): Wire {
...
describe("applyCatch", () => {
  test("returns undefined when no catch handler", async () => {
    const ctx = makeCtx();
    const w = makeWire([{ expr: { type: "ref", ref: REF } }]);
    assert.equal(await applyCatch(ctx, w), undefined);
  });
  ...
  test("applies catch value constant", async () => {
    ...
    assert.equal(await applyCatch(ctx, w), "fallback");
  });
  ...
  test("resolves catch ref", async () => {
    ...
    assert.equal(await applyCatch(ctx, w), "backup-value");
  });
  ...
  test("applies catch control — continue", async () => {
    ...
    assert.equal(await applyCatch(ctx, w), CONTINUE_SYM);
  });
  ...
  test("applies catch control — break", async () => {
    ...
    assert.equal(await applyCatch(ctx, w), BREAK_SYM);
  });
  ...
  test("catch control — throw", async () => {
    ...
    await assert.rejects(() => applyCatch(ctx, w), /catch-throw/);
  });
});
