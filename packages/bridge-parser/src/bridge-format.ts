...
function serBodyExpr(
  ...
  switch (expr.type) {
    ...
    case "literal": {
      ...
      // String — use formatExprValue to quote identifier-like strings
      return formatExprValue(v as string);
    }
    case "ternary": {
...
function serBodyCatch(c: WireCatch | undefined, ctx: BodySerContext): string {
  ...
  if (typeof v === "object" && v !== null) return ` catch ${JSON.stringify(v)}`;
  if (typeof v === "string") {
    return ` catch ${formatExprValue(v)}`;
  }
  return ` catch ${v}`;
}
    ...
    switch (stmt.kind) {
      ...
      case "wire": {
        ...
        if (
          ...
          const v = stmt.sources[0]!.expr.value;
          if (typeof v === "string") {
            if (needsQuoting(v)) {
              lines.push(`${indent}${target} = "${v}"`);
            } else {
              lines.push(`${indent}${target} = ${formatBareValue(v)}`);
            }
          } else if (typeof v === "number" || typeof v === "boolean") {
            lines.push(`${indent}${target} = ${v}`);
          } else if (v === null) {
            lines.push(`${indent}${target} = null`);
          } else {
            lines.push(`${indent}${target} = ${JSON.stringify(v)}`);
          }
        } else {
 ...
 */
function serializeBridgeBlockFromBody(bridge: Bridge): string {
 ...
 */
function serializeDefineBlockFromBody(def: DefineDef): string {
 ...
 */
function serializeToolBlockFromBody(tool: ToolDef): string {
...
function serializeToolBlock(tool: ToolDef): string {
  if (tool.body) return serializeToolBlockFromBody(tool);
...
function serializeDefineBlock(def: DefineDef): string {
  if (def.body) return serializeDefineBlockFromBody(def);
...
function serializeBridgeBlock(bridge: Bridge): string {
  if (bridge.body) return serializeBridgeBlockFromBody(bridge);
