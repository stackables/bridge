...
describe("parseBridge", () => {
  test("simple bridge with input handle", () => {
    ...
    assert.equal(instr.kind, "bridge");
    assert.equal(instr.type, "Query");
    assert.equal(instr.field, "geocode");
    assert.equal(instr.handles.length, 3);
    assertDeepStrictEqualIgnoringLoc(instr.handles[0], {
      handle: "gc",
      kind: "tool",
      name: "hereapi.geocode",
    });
    assertDeepStrictEqualIgnoringLoc(instr.handles[1], {
      handle: "i",
      kind: "input",
    });
    assertDeepStrictEqualIgnoringLoc(instr.handles[2], {
      handle: "o",
      kind: "output",
    });
  ...
  test("nested output paths", () => {
    ...
    assertDeepStrictEqualIgnoringLoc((instr.wires[1] as PullWire).from.path, [
      "properties",
      "0",
      "location",
      "city",
...
describe("parser robustness", () => {
  ...
  test("with tool keyword is case-insensitive", () => {
    ...
    const toolHandle = instr.handles.find((h) => h.kind === "tool");
  ...
  test("with context keyword is case-insensitive", () => {
    ...
    assert.notEqual(
      instr.handles.find((h) => h.kind === "context"),
      undefined,
    );
  });

  test("element mapping works with tab indentation", () => {
    const instr = parseBridge(
      "version 1.5\nbridge Query.search {\n\twith hereapi.geocode as gc\n\twith input as i\n\twith output as o\n\ngc.q <- i.search\no.results <- gc.items[] as item {\n\t.lat <- item.position.lat\n\t.lng <- item.position.lng\n}\n}\n",
    ).instructions.find((i) => i.kind === "bridge") as Bridge;
    assert.equal(
      instr.wires.filter((w) => "from" in w && w.from.element).length,
      2,
    );
  });
...
describe("brace block syntax", () => {
  test("bridge block with braces parses identically to indented style", () => {
    const brace = parseBridge(bridge`
      version 1.5
    ...
    `);
    const indent = parseBridge(bridge`
      version 1.5
...
describe("version tags: parser produces version on HandleBinding", () => {
  test("bridge handle with @version stores version string", () => {
    ...
    const toolHandle = instr.handles.find(
