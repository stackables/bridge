 ...
 * Expression wires (`<-` with operator) evaluate a binary expression at runtime.
...
export type Wire =
  ...
  | { expr: { op: ExprOp; left: ExprOperand; right: ExprOperand }; to: NodeRef; force?: true };

/** Supported binary operators for expression wires */
export type ExprOp = '*' | '/' | '+' | '-' | '==' | '!=' | '>' | '>=' | '<' | '<=';

/** An operand in a binary expression — either a resolved source reference or a literal value */
