...
| `with myTool as t memoize` | Same as above, but reuses the cached result when that handle sees the same resolved inputs again. |
...
`memoize` is valid only for tool handles. Each declared memoized handle owns its
own cache.
...
<MiniPlaygroundModal client:only="react" exampleId="tool-inheritance" autoRun={true} />
...
<MiniPlaygroundModal client:only="react" exampleId="const-blocks" autoRun={true} />
...
<MiniPlaygroundModal client:only="react" exampleId="define-blocks" autoRun={true} />
