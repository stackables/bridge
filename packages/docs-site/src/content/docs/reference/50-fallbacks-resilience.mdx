...
2. If none return truthy data, it moves to Computed (Cost 1) sources.
3. It stops and returns the very first result it finds.

In the example above, if the user provides `city` in their GraphQL arguments, the engine reads it instantly from memory (`Cost 0`). The expensive geocoding API (`Cost 1`) is **never called**.