...
| Feature                      | Example                                                  |
| ---------------------------- | -------------------------------------------------------- |
| **`alias` declarations**     | `alias d <- api.result.data`                             |
| **Safe-exec aliases** (`?.`) | `alias safeVal <- api?.value`                            |
| **Alias fallbacks**          | `alias x <- (expr) ? val : null ?? panic "msg"`          |
...
| **Loop-scoped tools**       | `o <- items[] as i { with api as t .id <- t.value }`  |
| **Memoized handles**        | `with std.httpCall as fetch memoize`                  |
