...
If the API returns 0 users, or 2+ users, the `first` tool will immediately throw an error. This allows you to catch the bad state with an Error Guard (`??`) and handle it gracefully!
