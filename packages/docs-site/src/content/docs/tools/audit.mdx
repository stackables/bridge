...
| :------------------ | :------- | :-------------------------------------------------------------------------------------- |
| `level`             | `"info"` | The log level method to call on your logger (`"debug"`, `"info"`, `"warn"`, `"error"`). |
| _(Any other field)_ | —        | Aggregated into a single structured JSON object and sent to the logger.                 |
...
If no logger is provided, the tool safely acts as a fast no-op.

/advanced/observability/#structured-logging
